...
///   TINYBROWSER_PROFILE=<index>   pin a specific profile from PROFILES
///   TINYBROWSER_ROTATE_PROFILE=1  pick a random profile per context
pub fn select_profile() -> &'static BrowserProfile {
    if let Some(idx) = std::env::var("TINYBROWSER_PROFILE")
        .ok()
        .as_deref()
        .map(str::trim)
    ...
    if env_enabled("TINYBROWSER_ROTATE_PROFILE") {
