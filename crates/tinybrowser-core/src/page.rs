...
/// Parse `TINYBROWSER_GEOLOCATION="lat,lon"` for the navigator.geolocation shim.
...
fn env_geolocation() -> Option<(f64, f64)> {
    let raw = std::env::var("TINYBROWSER_GEOLOCATION").ok()?;
...
fn default_navigation_timeout() -> std::time::Duration {
    navigation_timeout_from_env_value(std::env::var("TINYBROWSER_NAV_TIMEOUT_MS").ok().as_deref())
}
        ...
        r#"(function() {{
            ...
            function syncSheet() {{
                if (!style) {{
                    ...
                    style.setAttribute('data-tinybrowser-external-stylesheets', '');
                    style.textContent = `{escaped_css}`;
                    globalThis.__tinybrowser_registerLinkedStylesheet(link, style);
                }}
    ...
    format!(
        r"(function() {{
            ...
            for (var i = 0; i < styles.length; i++) {{
                ...
                if (candidate.hasAttribute('data-tinybrowser-external-stylesheets')
                    || candidate.hasAttribute('data-tinybrowser-inline-import')) continue;
                authorIndex++;
            ...
            imported.setAttribute('data-tinybrowser-inline-import', '');
...
impl Page {
    ...
    /// value takes precedence over `TINYBROWSER_NAV_TIMEOUT_MS`; callers that do
    ...
    async fn execute_scripts_with_module_budget(&mut self, module_budget_override: Option<u64>) {
        ...
        // synchronous hang. Raise it further with TINYBROWSER_SCRIPT_DEADLINE_MS=<ms>
        // for very heavy SPAs on slow networks (pair it with a matching client
        // navigation timeout).
        let script_deadline_ms: u64 = std::env::var("TINYBROWSER_SCRIPT_DEADLINE_MS")
            .ok()
            ...
            .map(|(idx, url)| {
                ...
                async move {
                    ...
                    if parsed.scheme() == "data" {
                        ...
                        let resp = tinybrowser_net::Response {
                            url: parsed,
                            status: 200,
                            headers,
                            body,
        ...
        let module_budget_ms: u64 = {
            ...
            let short_ms: u64 = module_budget_override.unwrap_or_else(|| {
                std::env::var("TINYBROWSER_MODULE_BUDGET_MS")
                    .ok()
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(3_000)
            });
        ...
        let module_hostcall_grace_ms = if module_budget_override.is_some() {
            ...
            std::env::var("TINYBROWSER_MODULE_HOSTCALL_GRACE_MS")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(5_000)
        };
        ...
        enum ScheduledScript {
            Classic(usize),
            Module {
                prepared: tinybrowser_js::runtime::PreparedModule,
                url: Option<String>,
        ...
        let execute_classic =
            |page: &mut Self,
             script: &ScriptInfo,
             fetched_script: Option<(String, String, tinybrowser_net::Response)>| {
    ...
    pub async fn navigate_with_wait_post(
        ...
        // with `TINYBROWSER_NAV_TIMEOUT_MS=NN`, or set a page-scoped deadline when
    ...
    pub async fn settle(&mut self, max_ms: u64) {
        ...
        if let Some(js) = &mut self.js {
            if std::env::var_os("TINYBROWSER_STRICT_SETTLE").is_some() {
    ...
    async fn settle_runtime_for_duration(js: &mut JsRuntime, duration_ms: u64) {
    ...
    async fn navigate_single(
        ...
        // waitUntil, so handlers that read window.__tinybrowser_css see it.
    ...
    pub async fn evaluate_for_cdp(
        ...
        await_promise: bool,
    ) -> tinybrowser_js::runtime::RemoteObjectInfo {
        if let Some(js) = &mut self.js {
            match js
    ...
    ) -> Result<tinybrowser_js::runtime::RemoteObjectInfo, String> {
        if let Some(js) = &mut self.js {
            ...
            Ok(tinybrowser_js::runtime::RemoteObjectInfo {
                js_type: match &value {
                    serde_json::Value::String(_) => "string".into(),
                    serde_json::Value::Number(_) => "number".into(),
                    serde_json::Value::Bool(_) => "boolean".into(),
    ...
    /// cached (e.g. it exceeded TINYBROWSER_NETWORK_BODY_BUFFER_BYTES and was
    ...
    pub fn enable_interception(
        ...
        let (tx, rx) =
            tokio::sync::mpsc::unbounded_channel::<tinybrowser_js::ops::InterceptedRequest>();
    ...
    pub fn set_intercept_tx(
        &mut self,
        tx: tokio::sync::mpsc::UnboundedSender<tinybrowser_js::ops::InterceptedRequest>,
    ) {
...
mod tests {
    ...
    async fn linked_stylesheet_graph_fetches_once_and_preserves_order_and_bases() {
        ...
        page.on_request(std::sync::Arc::new(move |request| {
            if request.resource_type == tinybrowser_net::ResourceType::Stylesheet {
        ...
        let sheets = page
            ...
            .unwrap()
            .with_dom(|dom| {
                dom.query_selector_all("style[data-tinybrowser-external-stylesheets]")
                    .unwrap()
                    .into_iter()
        ...
        for path in ["/a.css", "/b.css"] {
            assert_eq!(
                observed_requests
                    ...
                    .collect::<Vec<_>>(),
                vec![tinybrowser_net::ResourceType::Stylesheet],
            ...
            page.js
                ...
                .evaluate(
                    "[document.readyState, globalThis.__postLoadDynamicRan === true, \
                     globalThis.__tinybrowser_hasPendingDynamicScripts(), \
                     globalThis.__tinybrowser_hasPendingLoadDelayingScripts()]",
                )
                .unwrap(),
    ...
    fn print_media_onload_can_activate_a_fetched_stylesheet() {
        ...
        let mut runtime = tinybrowser_js::runtime::JsRuntime::new();
        runtime.set_dom(dom);
        ...
        let state = runtime
            .with_dom(|dom| {
                let link = dom
                    ...
                    .expect("async link");
                let styles = dom
                    .query_selector_all("style[data-tinybrowser-external-stylesheets]")
                    .expect("valid selector");
                (
                    dom.get_node(link)
        ...
        let state = runtime
            .with_dom(|dom| {
                ...
                (
                    dom.get_node(link)
                        .and_then(|node| node.get_attribute("data-loaded").map(str::to_owned)),
                    dom.query_selector("style[data-tinybrowser-external-stylesheets]")
                        .expect("valid selector")
                        .and_then(|style| {
                            dom.get_node(style)
            ...
            .evaluate(
                ...
                (() => {
                    ...
                    const source = document.querySelector(
                        'style[data-tinybrowser-external-stylesheets]'
                    );
                    return {
