...
//!      and parks on resolve_rx.await INSIDE the js_lock guard)
...
//!
//! Run with `cargo test -p tinybrowser-cdp --test concurrent_navigations_with_fetch
//! -- --nocapture --ignored`.
    ...
    local
        .run_until(async {
            tokio::task::spawn_local(async move {
                let _ = tinybrowser_cdp::server::start(port).await;
            });
            tokio::task::spawn_local(async move {
