...
impl CdpContext {
    ...
    pub fn new_with_shared_context(default_context: Arc<BrowserContext>) -> Self {
        ...
        CdpContext {
            ...
            js_lock: Arc::new(tokio::sync::Mutex::new(())),
        }
    }
...
pub async fn dispatch(req: &CdpRequest, ctx: &mut CdpContext) -> CdpResponse {
    ...
    // self-bounds via TINYBROWSER_NAV_TIMEOUT_MS plus the watchdog-bounded settle.
    let cmd_budget_ms: u64 = std::env::var("TINYBROWSER_CDP_COMMAND_TIMEOUT_MS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(60_000);
    let cmd_watchdog = if cmd_budget_ms == 0 || is_js_free_method(&req.method) {
        None
    } else {
