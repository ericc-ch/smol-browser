...
pub async fn handle(
    ...
    match method {
        ...
        "getProperties" => {
            ...
            //      __tinybrowser_objects so later callFunctionOn can resolve it.
...
/// context tinybrowser has not advertised. Returns `Ok(())` when the parameter is
...
mod tests {
    ...
    /// target exists. Real Chrome replies with `{}`; before the fix tinybrowser
