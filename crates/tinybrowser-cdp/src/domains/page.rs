...
pub(crate) fn paint_unsupported(method: &str) -> String {
    format!(
        "Page.{method} is not supported by tinybrowser: no layout or paint engine. \
         For visual snapshots, drive a real headless Chromium for the \
         screenshot leg of your pipeline and use tinybrowser for the scraping leg."
    )
}
...
pub fn emit_navigation_events(
    ...
    page_url: &str,
    page_id: &str,
    network_events: &[tinybrowser_core::NetworkEvent],
...
mod tests {
    ...
    fn runtime_network_events_reuse_the_document_loader_without_lifecycle_replay() {
        ...
        let event = tinybrowser_core::NetworkEvent {
    ...
    fn assert_paint_unsupported(err: &str, method: &str) {
        assert!(
            !err.contains("Unknown Page method"),
            "{method} must NOT fall through to the catch-all: {err}"
        );
        assert!(
            !err.contains("render feature"),
            "{method} must not mention a deleted render feature: {err}"
        );
        assert!(
            err.contains("no layout or paint engine"),
            "{method} must say there is no paint engine: {err}"
        );
        assert!(
    ...
    async fn print_to_pdf_is_explicit_without_a_renderable_session() {
        ...
        assert_paint_unsupported(&err, "printToPDF");
    }
    ...
    async fn capture_screenshot_returns_descriptive_unsupported_error() {
        ...
        assert_paint_unsupported(&err, "captureScreenshot");
        let err2 = handle("captureSnapshot", &json!({}), &mut ctx, &None)
            .await
            .expect_err("captureSnapshot must error until a real renderer exists");
        assert_paint_unsupported(&err2, "captureSnapshot");
    }

    #[tokio::test]
    async fn screencast_methods_return_descriptive_unsupported_errors() {
        let mut ctx = CdpContext::new();
        for method in ["startScreencast", "stopScreencast", "screencastFrameAck"] {
            let err = handle(method, &json!({}), &mut ctx, &None)
                .await
                .expect_err("screencast methods must error without a paint engine");
            assert_paint_unsupported(&err, method);
        }
    }
