...
impl ResourceRequest {
    ...
    pub(crate) fn accept(&self) -> &'static str {
        match self.resource_type {
            ...
            // AVIF is intentionally omitted until tinybrowser's decoder can paint
...
/// `TINYBROWSER_ALLOW_PRIVATE_NETWORK`) the lookup passes through unfiltered.
...
pub struct HttpClient {
    ...
    /// through in addition to the `TINYBROWSER_ALLOW_PRIVATE_NETWORK` env var.
...
impl HttpClient {
    ...
    /// (if any). Exposed so callers outside the `tinybrowser-net` crate — notably
    /// `op_fetch_url` in `tinybrowser-js` (#139) — can route their own reqwest
