    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            std::thread::Builder::new()
                ...
                .expect("spawn obscura-js network recv thread");
        ...
        .expect("spawn obscura-js network thread");
}

struct QjsModuleHost {
    state: std::rc::Weak<RefCell<ops::ObscuraState>>,
...
impl QuickJsRuntime {
    ...
    fn install_ops_in<'js>(&self, ctx: Ctx<'js>) -> Result<(), String> {
        ...
        ctx.eval::<(), _>(
            r#"
            globalThis.Deno = { core: { ops: __obscura_ops } };
            delete globalThis.__obscura_ops;
            "#,
        )
        .catch(&ctx)
        .map_err(|e| e.to_string())?;
    ...
    pub fn eval_module(
        ...
        self.context.with(|ctx| {
            ...
            ctx.globals()
                .set("__obscura_mod_p", promise)
                .map_err(|e| e.to_string())?;
            ctx.eval::<(), _>(
                r#"
                globalThis.__obscura_mod_state = 0;
                globalThis.__obscura_mod_err = null;
                globalThis.__obscura_mod_p.then(
                    function() { globalThis.__obscura_mod_state = 1; },
                    function(e) {
                        globalThis.__obscura_mod_state = 2;
                        globalThis.__obscura_mod_err = (e && e.message) ? String(e.message) : String(e);
                    }
                );
                delete globalThis.__obscura_mod_p;
                "#,
            )
            .catch(&ctx)
        ...
        loop {
            ...
            let state = self
                .context
                .with(|ctx| ctx.eval::<i32, _>("globalThis.__obscura_mod_state|0").unwrap_or(0));
            if state == 1 {
                return Ok(());
            }
            if state == 2 {
                let err = self
                    .context
                    .with(|ctx| {
                        ctx.eval::<String, _>("String(globalThis.__obscura_mod_err || 'module error')")
...
mod tests {
    ...
    fn setup_at(html: &str, page_url: &str) -> QuickJsRuntime {
        ...
        rt.set_url(page_url);
        rt.set_dom(obscura_dom::parse_html(html));
    ...
    fn allow_private_client() -> std::sync::Arc<obscura_net::ObscuraHttpClient> {
        std::sync::Arc::new(obscura_net::ObscuraHttpClient::with_full_options(
