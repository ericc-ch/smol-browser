...
(function _preHideInternals() {
  var _names = [
    ...
    '__tinybrowser_errors', '__tinybrowser_init', '__tinybrowser_hide_list',
    '__tinybrowser_objects', '__tinybrowser_oid', '__tinybrowser_ua',
    '__tinybrowser_platform', '__tinybrowser_ua_platform', '__tinybrowser_ua_platform_version',
    '__tinybrowser_stealth', '__tinybrowser_markTrusted',
    '__tinybrowser_registerLinkedStylesheet',
    '__markParserScripts', '__tinybrowser_hasPendingDynamicScripts',
    '__tinybrowser_hasPendingLoadDelayingScripts',
    '__tinybrowser_nextPendingTimeoutDelay',
    '__tinybrowser_hw', '__tinybrowser_mem',
...
globalThis.__tinybrowser_errors = [];
...
globalThis.onerror = function(msg, src, line, col, error) {
  globalThis.__tinybrowser_errors.push({msg: String(msg), src: String(src||""), line, error: String(error||"")});
};
...
// unusualWindowProperties: tinybrowser's internal globals are made non-enumerable
// (see _preHideInternals and __tinybrowser_init), which hides them from
...
// __tinybrowser_hide_list, precomputed at snapshot-build time; referencing it lazily
...
(function _hideInternalsFromReflection() {
  ...
  function _set() {
    var list = globalThis.__tinybrowser_hide_list;
    if (!list) { return null; }
    if (_cache && _cacheLen === list.length) { return _cache; }
    _cache = new Set(list);
    _cache.add('__tinybrowser_hide_list');
...
Object.defineProperty(globalThis, '__tinybrowser_hasPendingDynamicScripts', {
  value: function() {
...
Object.defineProperty(globalThis, '__tinybrowser_hasPendingLoadDelayingScripts', {
...
globalThis.__tinybrowser_registerLinkedStylesheet = _registerLinkedStylesheet;
...
async function _loadLinkedStylesheet(c) {
  // tinybrowser does not yet reflect the `rel` IDL attribute back to the content
  ...
  try {
    ...
    style.setAttribute("data-tinybrowser-linked", fullUrl);
...
function _getFp() {
  ...
  const _uaPlat = globalThis.__tinybrowser_ua_platform || 'Windows';
...
const __tinybrowserPendingTimeoutDeadlines = new Map();
Object.defineProperty(globalThis, '__tinybrowser_nextPendingTimeoutDelay', {
  value: function() {
    const now = performance.now();
    let nearest = Infinity;
    for (const deadline of __tinybrowserPendingTimeoutDeadlines.values()) {
...
globalThis.setTimeout = (fn, delay = 0, ...args) => {
  ...
  const nativeId = _scheduleAfter(normalizedDelay, () => {
    ...
    __tinybrowserPendingTimeoutDeadlines.delete(id);
    if (_clearedTimers.has(id)) return;
    try { f(...args); } catch(e) { console.error("Timer error:", e); }
  });
  if (nativeId !== undefined) {
    _nativeTimerIds.set(id, nativeId);
    __tinybrowserPendingTimeoutDeadlines.set(id, performance.now() + normalizedDelay);
  }
  return id;
};
...
class Element extends Node {
  ...
  focus() { globalThis.__tinybrowser_focused = this; globalThis.__tinybrowser_click_target = this; }
  blur() { if (globalThis.__tinybrowser_focused === this) globalThis.__tinybrowser_focused = null; }
  ...
  // the DOM. See __tinybrowser_setInputFiles (issue #359).
  get files() {
  ...
  // deliberately retains tinybrowser's compatibility geometry.
  _renderBoxGeometry() {
  ...
  _rectFromRenderGeometry(geometry) {
    ...
    Object.defineProperty(rect, "__tinybrowserViewportFixed", {
  ...
  set scrollTop(v) {
    ...
    if (changed &&
        typeof globalThis.__tinybrowser_recompute_intersections === "function") {
  ...
  getBoundingClientRect() {
    globalThis.__tinybrowser_click_target = this;
  ...
  scrollIntoView(arg) {
    ...
    if (rect.__tinybrowserViewportFixed) return;
...
// Document in __tinybrowser_init) naturally restores the URL host.  Detached
...
class Document extends Node {
  ...
  get activeElement() { return globalThis.__tinybrowser_focused || this.body; }
...
function _chromeMajor() {
  var m = (globalThis.__tinybrowser_ua || '').match(/Chrome\/(\d+)/);
  return m ? (m[1] | 0) : 145;
}
...
globalThis.navigator = {
  ...
  userAgentData: {
    ...
    get platform() { return globalThis.__tinybrowser_ua_platform || "Windows"; },
    getHighEntropyValues(hints) {
      ...
      return Promise.resolve({
        ...
        mobile: false,
        model: "",
        platform: globalThis.__tinybrowser_ua_platform || "Windows",
        platformVersion: globalThis.__tinybrowser_ua_platform_version || "15.0.0",
  ...
  geolocation: {
    getCurrentPosition(success, error) {
      const coords = {
        latitude: (globalThis.__tinybrowser_geo_lat ?? 50.1109) + (_fpRand(500) - 0.5) * 0.1,
        longitude: (globalThis.__tinybrowser_geo_lon ?? 8.6821) + (_fpRand(501) - 0.5) * 0.1,
    ...
    watchPosition(success, error) {
      if (typeof success === 'function') {
        const coords = {
          latitude: (globalThis.__tinybrowser_geo_lat ?? 50.1109) + (_fpRand(503) - 0.5) * 0.1,
          longitude: (globalThis.__tinybrowser_geo_lon ?? 8.6821) + (_fpRand(504) - 0.5) * 0.1,
...
// Getters read __tinybrowser_* lazily (snapshot vs per-page) and are _markNative'd.
(function() {
  ...
  defGetter('userAgent', function() {
    return globalThis.__tinybrowser_ua ||
  ...
  defGetter('platform', function() {
    return globalThis.__tinybrowser_platform || "Win32";
  });
  ...
  // Values set per-page by __tinybrowser_init (avoids own data props on navigator).
  defGetter('hardwareConcurrency', function() { return globalThis.__tinybrowser_hw || 8; });
  defGetter('deviceMemory', function() { return globalThis.__tinybrowser_mem || 8; });
...
globalThis.__tinybrowser_set_screen_override = function(w, h, emulated) {
  globalThis.__tinybrowser_screen_emulated = !!emulated;
  if (Number.isFinite(w) && Number.isFinite(h) && w > 0 && h > 0) {
    globalThis.__tinybrowser_screen_w = w;
    globalThis.__tinybrowser_screen_h = h;
    _applyScreenSize(w, h, !!emulated);
    return;
  }
  delete globalThis.__tinybrowser_screen_w;
  delete globalThis.__tinybrowser_screen_h;
...
function _bodyToUint8Array(body) {
  ...
  // tinybrowser's Blob materializes its data into _bytes in the constructor.
...
function _installWasmStreamingFallback() {
  ...
  if (WebAssembly.instantiateStreaming && WebAssembly.instantiateStreaming.__tinybrowserFallback) return;
  ...
  fallback.__tinybrowserFallback = true;
...
globalThis.fetch = async (input, init = {}) => {
  ...
  if (parsed.requestId) {
    Object.defineProperty(response, "__tinybrowserRequestId", {
...
if (typeof URL === 'undefined' || !URL.prototype || !URL.__tinybrowser) {
...
globalThis.__tinybrowser_recompute_resizes = _scheduleResizeRenderCheckpoint;
...
function _styleElementIsCssomBridge(style) {
  return style.hasAttribute("data-tinybrowser-adopted")
    || style.hasAttribute("data-tinybrowser-linked")
    || style.hasAttribute("data-tinybrowser-external-stylesheets")
    || style.hasAttribute("data-tinybrowser-inline-import");
}
...
function _syncAdoptedStyles(root) {
  ...
  for (const sheet of sheets) {
    ...
    if (!node || node.parentNode !== target) {
      ...
      node.setAttribute("data-tinybrowser-adopted", "");
...
globalThis.__notifyMutation = function(type, target_nid, addedNodes, removedNodes, attributeName, oldValue) {
  ...
  // MutationObserver fired in tinybrowser, ever, despite the call sites being
...
globalThis.__tinybrowser_shadowHostNames = new Set(['article','aside','blockquote','body','div','footer','h1','h2','h3','h4','h5','h6','header','main','nav','p','section','span']);
...
(function() {
  ...
  globalThis.__tinybrowser_recompute_intersections = _scheduleIntersectionRenderCheckpoint;
...
// tinybrowser's CDP input pipeline marks its synthetic events via the
// non-enumerable __tinybrowser_markTrusted helper.
const _trustedEvents = new WeakSet();
globalThis.__tinybrowser_markTrusted = function(ev) { try { if (ev) _trustedEvents.add(ev); } catch (_e) {} return ev; };
...
// direct assignment.
globalThis.__tinybrowser_setFieldValue = function(el, field, value) {
  try {
...
globalThis.__tinybrowser_setInputFiles = function(el, specs) {
  ...
  try { el.dispatchEvent(globalThis.__tinybrowser_markTrusted(new Event("input", { bubbles: true }))); } catch (_e) {}
  try { el.dispatchEvent(globalThis.__tinybrowser_markTrusted(new Event("change", { bubbles: true }))); } catch (_e) {}
};
...
// Conservative XML well-formedness check. tinybrowser has no XML parser, so this
...
globalThis.DOMParser = class DOMParser {
  parseFromString(source, mimeType) {
    ...
    // Chrome. tinybrowser has no XML parser, so the tree stays HTML-parsed.
...
function _structuredClone(value, seen) {
  ...
  if (typeof value[Symbol.toStringTag] === "string" && globalThis.__tinybrowser_clone_hooks) {
    const hook = globalThis.__tinybrowser_clone_hooks[value[Symbol.toStringTag]];
...
function _cssSupportsColor(value) {
  ...
  if (/^rgba?\(/.test(lower) && lower.endsWith(")")) {
    ...
    // tinybrowser yet and therefore must not select an unsupported @supports arm.
...
// True when the document was loaded from an XML/XHTML source. tinybrowser has no
...
Element.prototype.attachShadow = function attachShadow(opts) {
  ...
  if (!globalThis.__tinybrowser_shadowHostNames.has(_ln) && _ln.indexOf('-') === -1) {
...
URL.createObjectURL = function(blob) {
  if (blob) {
    const id = 'blob:tinybrowser/' + Math.random().toString(36).substring(2);
    ...
    // The tinybrowser Blob materializes _bytes in its constructor; fall back to
  ...
  return 'blob:tinybrowser/fallback';
};
...
if (!globalThis.crypto.subtle) {
  ...
  globalThis.__tinybrowser_clone_hooks = globalThis.__tinybrowser_clone_hooks || {};
  ...
  globalThis.__tinybrowser_clone_hooks["CryptoKey"] = function (src, seen) {
...
if (typeof BroadcastChannel === 'undefined') {
  ...
  // tinybrowser page is one origin-bound browsing context today, so every channel
...
globalThis.__tinybrowser_init = function() {
  ...
  const sw = Number.isFinite(globalThis.__tinybrowser_screen_w) && globalThis.__tinybrowser_screen_w > 0
    ? globalThis.__tinybrowser_screen_w : scr[0];
  const sh = Number.isFinite(globalThis.__tinybrowser_screen_h) && globalThis.__tinybrowser_screen_h > 0
    ? globalThis.__tinybrowser_screen_h : scr[1];
  ...
  const vw = Number.isFinite(globalThis.__tinybrowser_viewport_w) && globalThis.__tinybrowser_viewport_w > 0
    ? globalThis.__tinybrowser_viewport_w : sw;
  const vh = Number.isFinite(globalThis.__tinybrowser_viewport_h) && globalThis.__tinybrowser_viewport_h > 0
    ? globalThis.__tinybrowser_viewport_h : sh - 80;
  _applyScreenSize(sw, sh, !!globalThis.__tinybrowser_screen_emulated);
  ...
  // standalone runtime has the same 1x default as tinybrowser's render surface.
  globalThis.devicePixelRatio = 1;
  globalThis.innerWidth = vw; globalThis.innerHeight = vh;
  globalThis.outerWidth = sw; globalThis.outerHeight = sh - 40;

  var hwValues = globalThis.__tinybrowser_stealth ? [4, 6, 8, 12, 16] : [2, 4, 6, 8, 12, 16];
  globalThis.__tinybrowser_hw = hwValues[Math.floor(_fpRand(400) * hwValues.length)];
  var memValues = globalThis.__tinybrowser_stealth ? [4, 8] : [0.25, 0.5, 1, 2, 4, 8];
  globalThis.__tinybrowser_mem = memValues[Math.floor(_fpRand(401) * memValues.length)];
  ...
  // Hide internals (_*, tinybrowser). The set of keys is static at
  ...
  const toHide = globalThis.__tinybrowser_hide_list || [];
  for (let i = 0; i < toHide.length; i++) {
    try { Object.defineProperty(globalThis, toHide[i], { enumerable: false }); } catch(e) {}
  }
  delete globalThis.__tinybrowser_init;
};
...
// for __tinybrowser_init to consume on every subsequent page. The snapshot
...
globalThis.__tinybrowser_hide_list = Object.getOwnPropertyNames(globalThis).filter(k =>
  k.startsWith('_') || k.includes('tinybrowser')
);
...
// tamperedFunctions: tinybrowser reimplements much of the DOM/Web platform in JS.
