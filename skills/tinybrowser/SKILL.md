...
```bash
CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=2 cargo build --release -p tinybrowser-cli --bins --features render
```
...
```bash
CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=2 cargo build --release -p tinybrowser-cli --bins --features render,stealth
```

Build without rendering when only DOM, extraction, or CDP automation is needed:

```bash
CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=2 cargo build --release -p tinybrowser-cli --bins --no-default-features
CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=2 cargo build --release -p tinybrowser-cli --bins --no-default-features --features stealth
```

Use `./target/release/tinybrowser` in the commands below when working from source.
...
```bash
tinybrowser --stealth fetch https://example.com --screenshot page.png
tinybrowser serve --stealth --port 9222
```
...
tinybrowser fetch https://example.com \
...
Run `tinybrowser mcp` for stdio or `tinybrowser mcp --http --port 3000` for HTTP.
...
tinybrowser supports many common layout and paint paths but is not a bundled Chrome
