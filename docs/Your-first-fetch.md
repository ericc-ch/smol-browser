`tinybrowser fetch` loads a URL, runs its JavaScript, and prints the result.
...
```bash
tinybrowser fetch https://news.ycombinator.com \
...
```bash
tinybrowser fetch https://example.com --eval "(function(){
...
```bash
tinybrowser fetch https://my-spa.example --wait-until domcontentloaded --eval "document.title"
```
...
(When driving tinybrowser via Puppeteer or Playwright the default is `domcontentloaded` to match client expectations.)
