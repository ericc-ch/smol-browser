...
### `TINYBROWSER_ALLOW_PRIVATE_NETWORK`
...
```bash
TINYBROWSER_ALLOW_PRIVATE_NETWORK=1 tinybrowser fetch http://localhost:8080
```

Per-process equivalent: `--allow-private-network` on any subcommand.

### `TINYBROWSER_NAV_TIMEOUT_MS`

Hard ceiling on a single navigation. Default 30000 (30 seconds). Applies to `Page.navigate` and the CLI `fetch` command.

```bash
TINYBROWSER_NAV_TIMEOUT_MS=60000 tinybrowser serve
```

### `TINYBROWSER_CDP_COMMAND_TIMEOUT_MS`

Per-command deadline for the CDP server. A hung page (a runaway `Runtime.evaluate`, a synchronous DOM op) is terminated after this budget so one bad session cannot hold the shared V8 lock and stall the others. Default 60000 (60 seconds); `0` disables it. Navigation self-bounds via `TINYBROWSER_NAV_TIMEOUT_MS` well under this.

```bash
TINYBROWSER_CDP_COMMAND_TIMEOUT_MS=30000 tinybrowser serve
```

### `TINYBROWSER_FETCH_TIMEOUT_MS`
...
```bash
TINYBROWSER_FETCH_TIMEOUT_MS=15000 tinybrowser serve
```

### `TINYBROWSER_PROXY`

Default proxy URL used by `tinybrowser-worker` for the parallel `scrape` command when no `--proxy` flag is set.

```bash
TINYBROWSER_PROXY=http://proxy.example.com:8080 tinybrowser scrape - < urls.txt
```
...
```bash
TINYBROWSER_TIMEZONE=America/New_York tinybrowser serve
```

### `TINYBROWSER_GEOLOCATION`

Override the coordinates the `navigator.geolocation` shim reports, as `lat,lon`. Without it the shim reports a fixed default. Keep it consistent with `TINYBROWSER_TIMEZONE` and the proxy region.

```bash
TINYBROWSER_GEOLOCATION="40.7128,-74.0060" tinybrowser serve
```
...
```bash
TINYBROWSER_PROFILE=2 tinybrowser serve
```

### `TINYBROWSER_ROTATE_PROFILE`
...
```bash
TINYBROWSER_ROTATE_PROFILE=1 tinybrowser serve
```

## MCP

### `TINYBROWSER_MCP_ALLOWED_ORIGINS`

Comma-separated `Origin` allowlist for the HTTP MCP transport (`tinybrowser mcp --http`). Off by default, which keeps the permissive behavior. When set, a browser request whose `Origin` is not listed is refused with `403` before it can drive the server; native, non-browser MCP clients (which send no `Origin`) are always allowed. Use it to stop cross-origin pages from reaching a loopback MCP port.

```bash
TINYBROWSER_MCP_ALLOWED_ORIGINS="https://app.example.com" tinybrowser mcp --http --host 0.0.0.0
```
...
`--verbose` on the CLI is equivalent to `RUST_LOG=tinybrowser=info`.
...
```bash
tinybrowser serve --v8-flags "--max-old-space-size=2048 --expose-gc"
```
...
tinybrowser does not honor `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY`. Use `--proxy` or `TINYBROWSER_PROXY`.
