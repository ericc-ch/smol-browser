`tinybrowser mcp` exposes tinybrowser as a Model Context Protocol server so MCP-capable clients (Claude Desktop, Claude Code, etc.) can drive it.
...
```bash
tinybrowser mcp --http --port 3000
```
...
```bash
tinybrowser mcp --stealth --proxy http://proxy.example.com:8080
```
...
Restart Claude Desktop. The tinybrowser tools appear in the tool list.

## Claude Code

```bash
claude mcp add tinybrowser /path/to/tinybrowser mcp
```
