...
```bash
tinybrowser fetch https://example.com --stealth
tinybrowser serve --stealth
tinybrowser scrape url1 url2 --stealth
tinybrowser mcp --stealth
```
...
Omit rendering with `cargo build --release -p tinybrowser-cli --bins --no-default-features --features stealth`.
...
```bash
tinybrowser fetch https://example.com --proxy http://proxy.example.com:8080
tinybrowser serve --proxy http://proxy.example.com:8080
```

With auth:

```bash
tinybrowser fetch https://example.com --proxy http://user:pass@proxy.example.com:8080
```

SOCKS5:

```bash
tinybrowser fetch https://example.com --proxy socks5://proxy.example.com:1080
```

## Custom User-Agent

```bash
tinybrowser fetch https://example.com --user-agent "Mozilla/5.0 (...) ..."
tinybrowser serve --user-agent "Mozilla/5.0 (...) ..."
```
...
```bash
TINYBROWSER_PROFILE=2 tinybrowser serve          # pin a specific profile by index
TINYBROWSER_ROTATE_PROFILE=1 tinybrowser serve   # random profile per browser context
```
