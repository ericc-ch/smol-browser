...
- Embeddable = spawn-and-drive binary, or the `obscura` lib crate as thin in-process wrapper. Keep the wrapper as-is; it costs zero binary size.
- CLI subcommands (`serve`/`fetch`/`scrape`/`mcp`): fate undecided; `serve` and `fetch` are load-bearing (e2e gate drives `fetch`).
