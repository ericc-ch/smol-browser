...
Copied from https://github.com/h4ckf0r0day/obscura-benchmark `obstacle-course/`
at `6ebac8293d7477f59e837768bfd4e74173f04f1c` (Apache-2.0). Owned in this repo;
no upstream sync.
...
Daily gate from the repo root (debug CLI, one run per stage):

```sh
e2e/run.sh
```

`observer-intersection` is `expected_fail`: it still runs, but a FAIL does not
